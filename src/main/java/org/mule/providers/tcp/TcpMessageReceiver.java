/*
 * $Id: TcpMessageReceiver.java 4219 2006-12-09 10:15:14Z lajos $
 * --------------------------------------------------------------------------------------
...
public class TcpMessageReceiver extends AbstractMessageReceiver implements Work {
    ...
    protected class TcpWorker implements Work, Disposable {
        ...
        protected byte[] processData(byte[] data) throws Exception {
            logger.debug("In ProcessData"+new String(data));
            ...
            if (returnMessage != null) {
                logger.debug("Return Message not null "+new String(returnMessage.getPayloadAsBytes()));
                return returnMessage.getPayloadAsBytes();
            } else {
                logger.debug("return Message is null");
                generateACK(new String(data),os,"","");
                return null;
            }
        }

        private void generateACK(String message, OutputStream os, String status, String error) throws Exception, IOException {
            ...
            //if (MllpMessageReceiver.this.connector.getSendACK()){
                String ackCode = "AA";
                String textMessage = "";
                ...
                try {
                    this.protocol.write(os, ACK.getBytes());//MllpMessageReceiver.this.connector.getCharsetEncoding()));
                } catch (Exception t) {
